...
| `PROP-TRD-004` | `cow-sdk-trading` | Client-side submission validator rejects every services-authority precondition (validTo range, zero amount, same-token, non-EthFlow native sentinel, zero-address from, metadata.signer mismatch, owner mismatch) before wire dispatch. Governed by [ADR 0015](docs/adr/0015-client-side-order-bounds-validator.md). | Contract | Yes | `crates/trading/tests/validation_contract.rs`, `crates/trading/src/validation.rs (tests)` | 2026-04-21 |
