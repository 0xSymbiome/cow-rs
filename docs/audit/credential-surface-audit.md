...
sub-metadata deserializer's redaction pattern. `validate_app_data_doc` returns
`Result<(), AppDataError>`, so the typed error is the whole failure surface
the validation path can render. A regression test at
