...
| [Bounded Response Reads Audit](bounded-response-reads-audit.md) | Standing audit | HTTP response reads across `cow-sdk-core` (including its `transport::policy` module), `cow-sdk-transport-wasm`, `cow-sdk-wasm`, and the `cow-sdk-contracts` signature decode path | Per-client `max_response_bytes` streaming bound measured on decoded bytes, non-retryable `ResponseTooLarge` classification, success and error-body coverage, signature hex pre-decode bound, and documented RPC/JS-callback/wallet/IPFS residuals | Current | 2026-06-12 |
