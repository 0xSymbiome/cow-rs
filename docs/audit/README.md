...
| Artifact | Type | Owning surface | Scope | Status | Last reviewed |
| --- | --- | --- | --- | --- | --- |
| [WASM Surface Audit](wasm-surface-audit.md) | Standing audit | `cow-sdk-wasm` TypeScript-callable crate, npm package layout, and callback runtime boundary | Pure-helper layering, runtime support matrix, JavaScript callback HTTP transport, facade package exports, and error posture | Current | 2026-06-04 |
...
| Artifact | Type | Owning surface | Scope | Status | Last reviewed |
| --- | --- | --- | --- | --- | --- |
| [Source-Lock Provenance Audit](source-lock-provenance-audit.md) | Standing audit | source-lock provenance and release preflight authority | Source-lock commit pins, upstream freshness disclosure, Alloy runtime/core dependency provenance, publication preflight metadata, historical snapshot scope, and refresh ownership | Current | 2026-06-04 |

## Orderbook Wire DTO Coverage

| Artifact | Type | Owning surface | Scope | Status | Last reviewed |
| --- | --- | --- | --- | --- | --- |
| [Wire DTO Coverage Audit](wire-dto-coverage-audit.md) | Standing audit | `cow-sdk-orderbook` DTO coverage | OpenAPI-vendored orderbook response DTO inventories, request-payload fixtures, field-level round trips, and response forward compatibility | Current | 2026-05-30 |
...
| [Credential Surface Audit](credential-surface-audit.md) | Standing audit | Credential-bearing builder storage, URL configuration, host-policy errors, public error diagnostics, wallet add-chain payloads, wasm error envelopes, and the SDK facade | Redacted credential storage plus sanitized host-policy failures, typed-redacted public error diagnostics, and redacted JS-visible `WasmError` diagnostics | Current | 2026-06-03 |
...
| [Error Classification Audit](error-classification-audit.md) | Standing audit | The `class()` accessors on the `cow-sdk` error family and the shared `cow_sdk_core::ErrorClass` | The shared `ErrorClass` partition and its facade re-export, the per-type `class()` accessor on every facade-family error type, the `SdkError::class()` delegation, composite granularity (a wrapped 429 stays `RateLimited`), and the typed-discriminant redaction posture | Current | 2026-05-31 |
