...
| [COW Shed Contract Bindings Audit](cow-shed-contract-bindings-audit.md) | Standing audit | Byte-identical COW Shed Solidity mirrors, proxy creation code, and deployment registry rows | Version-keyed proxy creation-code artifacts, digest validation, deployment rows, and factory ABI evidence | Current | 2026-05-26 |
...
| [Alloy Signer Adapter Audit](alloy-signer-adapter-audit.md) | Standing audit | `cow-sdk-alloy-signer` `LocalAlloyKeystoreSigner`, its builder, and its `Signer` implementation | Local-keystore message and typed-data signing, primary-type preservation, signature normalization, provider-required transaction methods, redacted errors, cancellation bridge, and dependency boundary | Current | 2026-05-26 |
| [Alloy Umbrella Adapter Audit](alloy-umbrella-adapter-audit.md) | Standing audit | `cow-sdk-alloy` `AlloyClient`, its builder, provider implementation, and owned signer handle | Wallet-filler composition, provider delegation, typed-data signing, broadcast-only transaction submission, no-broadcast raw signing deferral, redaction, cancellation, and dependency boundary | Current | 2026-05-26 |
...
| [Log-Provider Capability Audit](log-provider-capability-audit.md) | Standing audit | `cow-sdk-core` `LogProvider` capability and the `cow-sdk-alloy-provider` implementation | `LogProvider: Provider` capability supertrait, the `LogQuery` / `RawLog` / `LogMeta` types, the single bounded-call `get_logs` entry, and the alloy `LogProvider` impl with its filter/log conversions | Current | 2026-05-29 |
...
| [Trading SDK Runtime Prerequisites Audit](trading-sdk-runtime-prerequisites-audit.md) | Standing audit | `cow-sdk-trading` ready-state `Trading` and helper-only `TradingHelpers` construction | Ready quote/post setup, validated `AppCode`, helper-only setup, and helper prerequisites | Current | 2026-05-28 |
...
| [Credential Surface Audit](credential-surface-audit.md) | Standing audit | Credential-bearing builder storage, URL configuration, host-policy errors, public error diagnostics, wallet add-chain payloads, wasm error envelopes, and the SDK facade | Redacted credential storage plus sanitized host-policy failures, typed-redacted public error diagnostics, and redacted JS-visible `WasmError` diagnostics | Current | 2026-06-01 |
