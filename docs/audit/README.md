...
| [EIP-1271 Verification Cache Audit](eip1271-verification-cache-audit.md) | Standing audit | The `Eip1271Cache` trait and its always-available `NoopEip1271Cache`, plus the WASM EIP-1271 payload parity | Signature-keyed positive-only caching, the single always-available no-op cache, and native/WASM/TS payload vector parity | Current | 2026-06-24 |
...
| [Fuzz Coverage Audit](fuzz-coverage-audit.md) | Standing audit | The `cow-sdk-fuzz` crate and its `cargo-fuzz` targets | Target inventory, per-target seed contract, stable-toolchain compile gate, property traceability, and the public-surface boundary | Current | 2026-06-21 |
