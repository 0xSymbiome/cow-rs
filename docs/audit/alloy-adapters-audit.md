...
| Error and cancellation | Error classes cover validation, transport, remote, signing, provider-required, pending-transaction, cancelled, and internal failures; sensitive detail is redacted; `From<Cancelled>` propagates cancellation through each error type | Conforms |
