...
| --- | --- | --- |
| Core and orderbook base URLs | Configured base-URL map values redact in public diagnostics and serialization while routing keeps raw URLs | Conforms |
| Subgraph base URLs | Optional base-URL map values redact and unsupported-chain `None` markers remain visible | Conforms |
| Browser wallet add-chain URLs | Public chain parameters redact URL vectors while the EIP-1193 payload uses raw URL bytes | Conforms |
| App-data IPFS URIs | IPFS URI config fields redact in public output while fetch and upload policies use raw URI bytes | Conforms |
...
still receives the exact URL strings required by EIP-3085.
