...
Refresh trigger: any new `expect`, `unwrap`, or `panic!` site on a path reachable from the published public API
Related docs:
...
| Panic allowlist | `.github/config/panic-allowlist.yaml` carries 45 reviewed item-path entries for accepted static-invariant panic sites | Conforms |
...
It currently contains 45 reviewed item-path entries. Each accepted
production panic site remains tied to a documented static invariant rather
than to caller-controlled input.
...
```text
rg -n "\\.expect\\(|\\.unwrap\\(|panic!" crates -g "*.rs"
