...
| Panic allowlist | `.github/config/panic-allowlist.yaml` carries 49 reviewed item-path entries covering 61 accepted static-invariant panic-bearing calls | Conforms |
...
| `crates/core/src/types/identity.rs` | `AppDataHash::to_cid` retains two static-invariant `expect` calls. The first wraps a `Multihash::<64>` around the 32-byte `AppDataHash` digest; the 32-byte width is a type invariant of `AppDataHash` (`#[repr(transparent)]` over `alloy_primitives::B256`), strictly less than the 64-byte multihash capacity, so the wrap cannot fail. The second renders the resulting CID through `Cid::to_string_of_base(Base::Base16Lower)`; `Base16Lower` is the cow-protocol canonical multibase and is unconditionally supported by the `cid` crate. Both call sites carry inline `// SAFETY` comments naming the invariant. |
...
It currently contains 49 reviewed item-path entries covering 61
