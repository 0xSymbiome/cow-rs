...
| Panic allowlist | `.github/config/panic-allowlist.yaml` carries 52 reviewed item-path entries covering 62 accepted static-invariant panic-bearing calls | Conforms |
...
| `crates/core/src/types/amount.rs:278`; `crates/core/src/types/amount.rs:613` | Explicit arithmetic-overflow contracts: `Amount::pow` routes through `checked_pow` so the inner `ruint::Uint::pow` (= `wrapping_pow`) overflow path becomes a debug panic with caller location, and `DecimalAmount::to_decimal_string` rejects pathological `decimals >= 78` values that would overflow the `10^decimals` scale (every supported ERC-20 ships `decimals <= 18`, so the bound is structurally satisfied). Callers needing infallible behaviour use `Amount::checked_pow` or `Amount::saturating_pow`. |
