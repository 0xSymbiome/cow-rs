...
This matrix maps the main TypeScript surfaces to the Rust workspace.

| TypeScript surface | Rust crate(s) | Status | Scope | Public evidence |
| --- | --- | --- | --- | --- |
| `@cowprotocol/cow-sdk` root entrypoint | `cow-sdk` | Implemented | Thin facade over the Rust SDK family; browser wallet is feature-gated | `crates/sdk/src/lib.rs`, `crates/sdk/tests/public_api.rs` |
...
| Subgraph queries | `cow-sdk-subgraph` | Implemented | Read-only totals, day-volume, hour-volume, and custom query execution in a separate crate | `crates/subgraph/tests/api_contract.rs`, `crates/subgraph/tests/query_contract.rs`, `examples/native/scenarios/subgraph_query_roundtrip.rs`, `examples/native/scenarios/subgraph_custom_query_roundtrip.rs` |
