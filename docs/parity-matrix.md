...
addresses. The upstream TypeScript `cowprotocol/cow-sdk` is prior art for the
trading consumer-workflow shape; it does not govern the Rust public API shape,
the wire format, or the on-chain shapes, and is not a pinned parity source. The
slippage convention cow-rs implements faithfully is documented in
[ADR 0066](adr/0066-trading-slippage-and-suggestion-policy.md). See
[Parity Sources](parity-sources.md#source-ownership) for the full ownership
split.
...
| Browser wallet integration | the cow-rs browser-wallet surface over the EIP-1193 provider seam | `cow-sdk-browser-wallet`, `cow-sdk` | `examples/wasm/cow-trader-dioxus/README.md`, `docs/validation-scope.md` | `crates/browser-wallet/tests/provider_contract.rs`, `crates/browser-wallet/tests/wallet_contract.rs`, the direct browser-bridge proof, and the canonical browser-wallet example |
