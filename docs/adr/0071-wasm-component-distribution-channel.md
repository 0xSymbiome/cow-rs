...
component. The contract carries a deterministic order-identity world with no host
imports and a stateful client world for the order lifecycle; both the WASI 0.2 and
WASI 0.3 lanes are in scope behind one shared implementation.
...
- The crate wraps the SDK crates and never forks protocol logic; the deterministic
  ...
  implementation.
- A native golden test pins the reference values, and the build asserts the
  component reproduces them through jco and a Wasmtime host.
...
- The WIT contract is versioned and snapshot-gated so it cannot drift silently.
- Outputs distribute only through OCI and GitHub Release, behind the existing
  pre-1.0 release trigger; never crates.io, and never Warg.
...
- The build target is `wasm32-wasip2`, `wit-bindgen` is pinned, and the component is
  built and parity-tested in a dedicated job.
