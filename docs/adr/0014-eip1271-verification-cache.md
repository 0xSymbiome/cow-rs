...
dependency on signing; `InMemoryEip1271Cache` lives in `cow-sdk-signing` behind
the opt-in `in-memory-cache` feature. Both are reachable through
`cow-sdk-signing::cache`.
...
within the TTL is never blocked by a stale negative entry, and a transient
network failure can never pin a signer into a rejected state. Putting the trait
in `cow-sdk-contracts` rather than `cow-sdk-signing` preserves the dependency
direction (signing depends on contracts, never the reverse).
...
- Public surface: `Eip1271Cache` is a positive-only set with
  ...
  and dependency-free; `InMemoryEip1271Cache` is behind the opt-in
  `in-memory-cache` feature (default off).
- Runtime and support: the cache records a probe only on `Ok(())`. A magic-value
  ...
  on-chain within the TTL is never blocked by a stale entry.
  `InMemoryEip1271Cache` expires entries after five minutes by default, so the
  cache never pretends to be an authoritative view of mutable on-chain state, and
  on-chain settlement re-checks the signature regardless.
- Validation and review: contract tests assert a `NoopEip1271Cache` miss, a TTL
  drop on the next probe past expiry, oldest-first eviction past capacity, and
  no-lost-write under concurrent inserts against the same key space. Every caller
  in the signing, trading, examples, and e2e surfaces compiles against the
  cache-required `verify_eip1271_signature_cached` shape.
- Cost: one trait, two canonical implementations, and one `parking_lot`
  dependency on `cow-sdk-signing` (only under `in-memory-cache`) for the
  `InMemoryEip1271Cache` lock. Callers that do not want caching pass
