...
| [0071](0071-wasm-component-distribution-channel.md) | Accepted | Distribute the WebAssembly Component as three OCI packages on `ghcr.io/0xsymbiome`, parallel to the wasm-bindgen npm lane; CI builds all three worlds and runs the engine golden, with cross-runtime parity testing not yet wired. |
...
- When an accepted ADR points to a standing audit as current-state proof, the
  audit reciprocates by naming the ADR in its `related` frontmatter.
  Cross-linking is expected to be reciprocal: every audit listed under an ADR's
  `**Proven by:**` block names that ADR in its `related` frontmatter, and every
  audit whose `related` frontmatter names an ADR is listed under that ADR's
  `**Proven by:**` block.
...
- ADR metadata lives in a YAML frontmatter block delimited by `---` at the very
  top of the file, above the `# ADR NNNN: …` heading. Each ADR is an Open
  Knowledge Format concept document, so its `type` and frontmatter make it
  queryable by external tools without a bespoke parser.
- Required keys: `type` (always `Decision Record`), `id` (the `ADR-NNNN`
  identifier), `title`, `description` (a one-line summary), `status`, and
  `timestamp` (ISO-8601; the last-reviewed date).
- `date`, `last_reviewed`, `authors` (a YAML list of handles, e.g.
  `authors: [0xSymbiotic]`), `tags` (a YAML list), and `related` (a YAML list of
  `ADR-NNNN` identifiers) are optional. Omit them when empty.
- Allowed `status` values are `Proposed`, `Proposed (deferred)`, `Accepted`,
