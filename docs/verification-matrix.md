...
| --- | --- | --- | --- |
| `cow-sdk-core` | Shared chain config, validated partner-route selection, domain types, runtime traits, and redacted API-context diagnostics | `config_contract.rs`, `types_contract.rs`, `traits_contract.rs`, `docs/audit/partner-api-routing-audit.md` | `cargo test -p cow-sdk-core` |
...
| `cow-sdk-subgraph` | Read-only GraphQL query construction, redacted production route identity, sanitized typed request-failure context, nested request-variable fidelity, typed responses, equivalent string-or-number scalar decoding, malformed-scalar failure boundaries, and source-schema evidence | `property_contract.rs`, `api_contract.rs`, `query_contract.rs`, `types_contract.rs`, `schema_source_contract.rs`, `docs/audit/dependency-gate-audit.md`, `docs/audit/credential-surface-contract-hygiene-audit.md` | `cargo test -p cow-sdk-subgraph` |
