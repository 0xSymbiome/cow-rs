...
| --- | --- | --- | --- |
| `cow-sdk-core` | Shared chain config, domain types, runtime traits, and redacted API-context diagnostics | `config_contract.rs`, `types_contract.rs`, `traits_contract.rs` | `cargo test -p cow-sdk-core` |
...
| `cow-sdk-orderbook` | Typed orderbook transport, retry/status behavior, DTO conversion, quote-request `appData` and pagination fidelity, malformed-payload failure boundaries, source-schema evidence, and redacted context-override diagnostics | `property_contract.rs`, `api_contract.rs`, `request_contract.rs`, `transform_contract.rs`, `types_contract.rs`, `schema_source_contract.rs` | `cargo test -p cow-sdk-orderbook` |
| `cow-sdk-trading` | Quote, post, allowance, approval, cancellation, slippage monotonicity, calldata boundary preservation, quote-request override precedence, quote-to-order orchestration, typed partner-fee public inputs, balance-semantics preservation, `TradingSdk` constructor parity, and recoverable-signature owner or signer validation | `property_contract.rs`, `quote_contract.rs`, `post_contract.rs`, `order_contract.rs`, `allowance_contract.rs`, `cancel_contract.rs`, `onchain_contract.rs`, `slippage_contract.rs`, `sdk_contract.rs`, `parity_contract.rs`, `docs/audit/trading-order-construction-integrity.md`, `docs/audit/credential-surface-contract-hygiene.md` | `cargo test -p cow-sdk-trading` |
