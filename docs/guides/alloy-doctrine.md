---
type: Doctrine
title: "Alloy Doctrine"
description: "Pre-1.0; binding for the v1.0 cut and forward."
...
|---|---|---|---|---|
| Inner-layer address type | `alloy_primitives::Address` | `cow-sdk-core` (via `Address` newtype), `cow-sdk-contracts`, `cow-sdk-signing`, `cow-sdk-orderbook`, `cow-sdk-trading`, `cow-sdk-alloy*`, `cow-sdk-js` | ADR 0052 | `repr(transparent)` newtype, bit-for-bit layout, zero-cost conversion via `From::from` and the `as_alloy` / `into_alloy` accessors. |
...
| sol! ABI bindings | `alloy_sol_types::sol!` macro | `cow-sdk-contracts` (canonical home), including the off-by-default `composable` feature-module | ADR 0012, [Canonical Contract Bindings](../principles/canonical-contract-bindings.md) | Declared inline with `sol!`, mirroring upstream Solidity pinned by commit in `parity/source-lock.yaml`; the GPv2 and eth-flow bindings are proven byte-for-byte by the parity fixtures under `parity/fixtures/`, and the `ComposableCoW` and TWAP bindings by the in-crate contract tests (`PROP-CON-026`). |
...
| `Redacted<T>` credential wrapper | `cow-sdk-core::redacted` | ADR 0025 (workspace url-redaction convention) + [Credential Redaction by Construction](../principles/credential-redaction-by-construction.md) | alloy types do not redact credentials; cow's Debug/Display/Serialize/panic-path renderings must emit only sanitized identity. |
...
| `JsCallbackHttpTransport` (Node/Deno/Workers callback transport) | `cow-sdk-js::exports::JsCallbackHttpTransport` | ADR 0010, ADR 0040 | Runtime-neutral JS callback transport; alloy ships no equivalent. |
...
|---|---|---|---|---|
| `Provider` (read-only chain RPC) | `cow_sdk_core::Provider` | `cow-sdk-alloy-provider` (native, read-only); on `wasm32`, the host wallet's EIP-1193 provider is reached through the `cow-sdk-js` contract-read callback | `alloy_provider::DynProvider<Ethereum>`, transport via `reqwest`, redacted URL via `Redacted<reqwest::Url>` | ADR 0024, ADR 0035 |
...
| Narrow capability traits (`TypedDataSigner`, `DigestSigner`) | `cow_sdk_core::{TypedDataSigner, DigestSigner}` | Callback-shaped adapters (`cow-sdk-js`) that expose a single signing operation | n/a — these are cow-owned shapes; alloy ships no peer | ADR 0024, ADR 0045 |
| `HttpTransport` (REST/GraphQL) | `cow_sdk_core::HttpTransport` | `ReqwestTransport` and `FetchTransport`, both target-gated inside `cow-sdk-core`; `cow_sdk_js::exports::JsCallbackHttpTransport` | `reqwest::Client` for native; `web_sys` global `fetch` for browser; JS callback for Node/Deno/Workers | ADR 0010, ADR 0013 |
...
| Wallet/provider/signer JS callback boundary | `cow_sdk_js` typed callbacks (`TypedDataSignerCallback`, `DigestSignerCallback`, `CustomEip1271Callback`, `ContractReadCallback`, `CowFetchCallback`) | `cow-sdk-js` | Wallet/provider semantics owned by the host JS — an EIP-1193 provider wraps into the typed-data callback — not by Rust types | ADR 0040, ADR 0045 |
