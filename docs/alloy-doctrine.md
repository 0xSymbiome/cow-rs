...
|---|---|---|---|---|
| Inner-layer address type | `alloy_primitives::Address` | `cow-sdk-core` (via `Address` newtype), `cow-sdk-contracts`, `cow-sdk-signing`, `cow-sdk-orderbook`, `cow-sdk-trading`, `cow-sdk-alloy*`, `cow-sdk-browser-wallet` | ADR 0052 | `repr(transparent)` newtype, bit-for-bit layout, zero-cost conversion via `From::from` and the `as_alloy` / `into_alloy` accessors. |
...
| Narrow capability traits (`Owner`, `TypedDataSigner`, `DigestSigner`) | `cow_sdk_core::{Owner, TypedDataSigner, DigestSigner}` | Callback-shaped adapters (`cow-sdk-browser-wallet`, `cow-sdk-wasm`) that expose a single signing operation | n/a — these are cow-owned shapes; alloy ships no peer | ADR 0024, ADR 0029, ADR 0045 |
...
- the cow-owned `checked_add`/`checked_sub`/`checked_mul`/`saturating_*` arithmetic methods that delegate to the inner integer (`Amount` exposes no operator overloads — arithmetic is explicit and overflow-checked).
...
- **`cow-sdk-wasm` import fences (`cargo check-source-fences`)** — the `wasm-no-alloy-family` fence forbids `cow-sdk-wasm` from referencing the native alloy adapter crates (`cow-sdk-alloy*`); alloy-core primitives (`alloy_primitives`, `alloy_sol_types`) are used directly for ABI and event decoding (ADR 0052).
...
- **a `cow-sdk-composable` crate landing as a workspace member** — ADR 0048 records that composable is deferred. When such a crate lands, the doctrine's Bucket 1 entries for `SolValue::abi_encode` and the Bucket 2 entries for Shape A / Shape B blob encoding become live; no doctrine amendment is needed because the rows are already authored for the deferred state. The migration team should re-validate the bucket assignments against the actual shipped code at the time of crate landing.
