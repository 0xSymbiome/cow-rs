...
| ABI encode tuple | `alloy_sol_types::SolValue::abi_encode` and `abi_encode_sequence` | `cow-sdk-signing` (Shape B, shipped); composable (Shape A, deferred per ADR 0048) | ADR 0050, ADR 0052 | Shape B (forwarder) ships today in `cow_sdk_signing::eip1271` via `abi_encode_sequence`; Shape A (Safe muxer) uses `SolCall::abi_encode` because the selector prefix is load-bearing, and is deferred with composable. |
...
| Wallet/provider/signer JS callback boundary | `cow_sdk_wasm` typed callbacks (`TypedDataSignerCallback`, `Eip1193RequestCallback`, `DigestSignerCallback`, `CustomEip1271Callback`, `CowFetchCallback`) | `cow-sdk-wasm` | EIP-1193 provider request semantics owned by the host JS, not by Rust types | ADR 0040, ADR 0045, ADR 0047 |
