...
| keccak256 hash | `alloy_primitives::keccak256` | `cow-sdk-contracts`, `cow-sdk-signing`, `cow-sdk-app-data`, `cow-sdk-cow-shed` | ADR 0052 | One independent oracle (`sha3::Keccak256::digest`) is retained inside `crates/contracts/src/order.rs` as a test-only parity reference; do not collapse (see the `keccak_word` test-oracle entry in Bucket 2 below). |
