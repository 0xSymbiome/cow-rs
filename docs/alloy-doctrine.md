...
| Format decimal units | `alloy_primitives::utils::format_units` | `cow-sdk-core::DecimalAmount::to_decimal_string` | ADR 0052 | Guarded delegation: the `decimals == 0` short-circuit is preserved (avoid the literal drop-in that breaks at `Unit::MAX`). |
...
| Hex runtime decode/encode | `alloy_primitives::hex::{decode, encode}` | `cow-sdk-core`, `cow-sdk-app-data` | ADR 0052 | Retires direct `hex.workspace = true` from `cow-sdk-core`. |
...
| `Eip1271VerificationCache` trait | `cow_sdk_contracts::eip1271` (defined), re-exported from `cow_sdk_signing::cache` | ADR 0014 | A safe-by-construction memoization boundary specific to EIP-1271 probes; alloy ships no equivalent. Default-off, explicit-cache-arg contract is the security invariant. |
...
Eight constraints plus four additional surfaces are verified as `DO NOT SWAP`. The CI grep gates that mechanize each fence live at `.github/workflows/never-swap-gates.yml`; the per-site `// DO NOT SWAP` comments live at the load-bearing call sites in `crates/contracts/`, `crates/core/`, `crates/signing/`, `crates/transport-policy/`, and `crates/transport-wasm/`.
