...
| sol! ABI bindings | `alloy_sol_types::sol!` macro | `cow-sdk-contracts` (canonical home), `cow-sdk-cow-shed`; composable bindings deferred per ADR 0048 | ADR 0012, [Canonical Contract Bindings](principles.md) | Declared inline with `sol!` and proven byte-for-byte by the parity fixtures under `parity/fixtures/`, mirroring upstream Solidity pinned by commit in `parity/source-lock.yaml`. |
...
| Address registry (deployment authority) | `cow-sdk-contracts::Registry` (const table in `crates/contracts/src/deployments/registry.rs`) | ADR 0012 | `(ContractId, SupportedChainId, CowEnv)` keyed; alloy ships no deployment authority for the CoW protocol. |
...
- **`cargo-metadata` negative-edge invariants** — `cow-sdk-signing ⇏ cow-sdk-trading`, `cow-sdk-cow-shed ⇏ cow-sdk-trading`; reverse-edge `cow-sdk-trading ⇒ cow-sdk-signing` (ADR 0051).
