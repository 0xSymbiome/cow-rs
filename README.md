...
its proof in the [Properties Registry](docs/properties/index.md) and held by release-gating
...
- **Evidence over adjectives** — every protocol transform is cross-checked
  ...
  fixtures in CI; see [Parity and Provenance](docs/guides/parity.md).
...
cow-sdk@0.1.0-alpha.10` does the same. JavaScript and TypeScript consumers install
the wasm bindings from npm:

```sh
npm install @symbiome-forge/cow-sdk-wasm@0.1.0-alpha.10
```
...
The facade exposes each surface as a named module reached on its module path
(`cow_sdk::trading::Trading`, `cow_sdk::core::Address`), matching the `alloy`,
`reqwest`, and `tower` convention. The consumer-facing modules:

| Module path | What it owns | Guide |
| --- | --- | --- |
| `cow_sdk::trading` | High-level quote-to-order workflows: the fluent `Trading::swap` lifecycle, limit and pre-sign posting, EthFlow, and cancellation | [Getting Started](docs/guides/getting-started.md) |
...
| `cow_sdk::core` | Shared domain types and runtime traits: `Address`, `Amount`, `SupportedChainId`, `Signer`, and the `HttpTransport` seam | [core crate](crates/core/README.md) |
| `cow_sdk::app_data` | App-data document generation, validation, and CID handling | [app-data crate](crates/app-data/README.md) |
| `cow_sdk::http` | Shared HTTP retry, rate-limit, jitter, and error-classification policy over the `HttpTransport` seam | [Transport](docs/guides/transport.md) |
...
| `cow_sdk::composable` | ComposableCoW conditional orders and TWAP, behind the off-by-default `composable` feature | [contracts crate](crates/contracts/README.md) |
| `cow_sdk::cow_shed` | cow-shed account-abstraction hooks, behind the off-by-default `cow-shed` feature | [contracts crate](crates/contracts/README.md) |
...
[Parity And Provenance](docs/guides/parity.md).
...
The canonical first-touch path is [Getting Started](docs/guides/getting-started.md).
The shipped crate family and deferred capability boundaries are listed in the
[First-Release Scope](docs/guides/parity.md#first-release-scope).

Install is one line (see [Install](#install) above): `cargo add
cow-sdk@0.1.0-alpha.10` for Rust, `npm install @symbiome-forge/cow-sdk-wasm@0.1.0-alpha.10`
...
- Public claims are backed by repository-visible tests, fixtures, and release
  documentation.
- Public evolution follows the [Forward-Compatible Public Surfaces](docs/principles/forward-compatible-public-surfaces.md),
  [Credential Redaction by Construction](docs/principles/credential-redaction-by-construction.md),
  [Cooperative Cancellation Coverage](docs/principles/cooperative-cancellation-coverage.md),
  and [Minimum-Viable Panic Surface](docs/principles/minimum-viable-panic-surface.md)
  principles.
...
| --- | --- |
| Verification and release posture | [Verification](docs/guides/verification.md) and [Release Checklist](docs/guides/release-checklist.md) define the maintained proof and publication contract. |
...
| Publication state | `cow-sdk` `0.1.0-alpha.10` is published on crates.io and `@symbiome-forge/cow-sdk-wasm` `0.1.0-alpha.10` on npm. This is a pre-release: the public surface is stabilizing toward `0.1.0` and may change between alpha versions, as [Getting Started](docs/guides/getting-started.md) and the [Release Checklist](docs/guides/release-checklist.md) describe. |
...
lives in the [Documentation Index](docs/index.md). Quick starts:

- [Getting Started](docs/guides/getting-started.md) — facade-first path to a signed order
- [Architecture](docs/guides/architecture.md) — crate ownership and public boundaries
- [Verification](docs/guides/verification.md) and [Parity And Provenance](docs/guides/parity.md) — proof classes and upstream authorities
- [cow-rs and the TypeScript SDK](docs/guides/comparison-with-typescript-sdk.md) — deferred scope and the guarantees Rust adds
...
- Surface-to-proof mapping lives in [Verification](docs/guides/verification.md)

The [MSRV policy](docs/guides/msrv-policy.md) defines when the workspace may raise
