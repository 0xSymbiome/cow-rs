...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let allowance = trading
        .cow_protocol_allowance(&client, &AllowanceParams::new(COW, OWNER))
        .await?;
    ...
    // 4. Build a pre-sign transaction; gas is estimated through the client and
    //    returned on the concrete `PreparedTransaction` (no optional fields).
