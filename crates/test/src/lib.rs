...
//! // `testing.sdk` is a real `Trading` wired to in-memory doubles. Drive it in
//! // your async test with `testing.signer` / `testing.provider`, then assert on
//! // what your code sent:
...
pub struct MockTrading {
    ...
    pub sdk: Trading,
    /// The injected orderbook double (clone of the one inside `sdk`); read
    /// [`MockOrderbook::recorded`] to assert what was sent.
