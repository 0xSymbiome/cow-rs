...
fn bench_calculate_total_fee(c: &mut Criterion) {
    let executed_fee_amount = "5000000000000000000";
    let executed_fee = "1500000000000000000";
    c.bench_function("calculate_total_fee", |b| {
        b.iter(|| {
            let total = calculate_total_fee(
                black_box(Some(executed_fee_amount)),
                black_box(Some(executed_fee)),
            )
            .expect("decimal inputs must sum without overflow");
            black_box(total);
        });
    });
}
