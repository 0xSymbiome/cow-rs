...
The `cow-rs` SDK compiled to WebAssembly for JavaScript and TypeScript. It builds
the same deterministic Rust protocol logic the native crates run — order signing,
...
`wasm32`, exposed through typed DTOs and explicit callbacks. It wraps the existing `cow-sdk-core`, `cow-sdk-contracts`,
...
The committed declaration snapshots under `crates/js/snapshots/` are an
...
a `#[wasm_bindgen]` export without wiring it through `crates/js/npm/src/*.ts` then
