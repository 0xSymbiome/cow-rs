...
//
// wasm-pack emits a byte-identical `cow_sdk_js_bg.wasm` for the `bundler` and
...
// `${__dirname}/cow_sdk_js_bg.wasm` path; the bundler glue uses a bundler
...
const WASM = "cow_sdk_js_bg.wasm";
const NODE_GLUE = "cow_sdk_js.cjs";
const WEB_GLUE = "cow_sdk_js.js";
const LOAD_MARKER = "`${__dirname}/cow_sdk_js_bg.wasm`";
...
for (const flavour of descriptor.flavours) {
  ...
  // `new URL('cow_sdk_js_bg.wasm', import.meta.url)` — a sibling path the dedupe
