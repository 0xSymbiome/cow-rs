...
use cow_sdk_core::{
    Address, AppDataHash, AppDataHex, OrderBalance, OrderKind, OrderModel, OrderUid,
    SupportedChainId, TypedDataDomain, settlement_contract_address,
};
...
pub fn hash_order(domain: &TypedDataDomain, order: &Order) -> Result<String, ContractsError> {
...
pub fn pack_order_uid_params(params: &OrderUidParams) -> Result<OrderUid, ContractsError> {
    let digest = parse_hex32(&params.order_digest, "orderDigest")?;
...
pub fn extract_order_uid_params(order_uid: &OrderUid) -> Result<OrderUidParams, ContractsError> {
    ...
    let order_digest = format!("0x{}", hex::encode(&bytes[..32]));
...
pub fn hash_order_for_contract(
    ...
    parse_hex32(&digest, "orderDigest")
}
...
fn order_struct_hash(order: &NormalizedOrder) -> Result<[u8; 32], ContractsError> {
    ...
    encoded.extend_from_slice(&encode_u256_str("sellAmount", &order.sell_amount)?);
    encoded.extend_from_slice(&encode_u256_str("buyAmount", &order.buy_amount)?);
    encoded.extend_from_slice(&encode_u32(order.valid_to));
    encoded.extend_from_slice(&parse_bytes32_hash(&order.app_data)?);
    encoded.extend_from_slice(&encode_u256_str("feeAmount", &order.fee_amount)?);
