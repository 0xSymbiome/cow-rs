...
impl AppDataHash {
    ...
    /// string for the same input digest; the round-trip pairs with
    /// [`AppDataHash::try_from_cid`].
    ///
    /// # Panics
    ///
    /// The two infallible `.expect` calls below are statically unreachable.
    /// `Multihash::<64>::wrap` accepts any slice whose length is at most
    /// 64 bytes; this digest is fixed at 32 bytes by the `AppDataHash`
    /// type invariant. `Cid::to_string_of_base(Base::Base16Lower)`
    /// returns `Err` only when the requested base is not supported, and
    /// `Base16Lower` is always supported by the `multibase` crate. Both
    /// panic sites are recorded in `.github/config/panic-allowlist.yaml`
    /// per the canonical panic-policy doctrine.
    #[must_use]
    pub fn to_cid(&self) -> String {
        const RAW_CODEC: u64 = 0x55;
        const KECCAK_256_CODE: u64 = 0x1b;

        // SAFETY: AppDataHash is #[repr(transparent)] over B256; its byte
        // slice is always exactly 32 bytes, strictly less than the 64-byte
        // capacity of Multihash<64>. Multihash::wrap can only return Err
        // when the digest exceeds capacity, so this branch is statically
        // unreachable.
        let hash = multihash::Multihash::<64>::wrap(KECCAK_256_CODE, self.as_slice())
            .expect("AppDataHash fits the 64-byte Multihash capacity by type invariant");
        let cid = cid::Cid::new_v1(RAW_CODEC, hash);

        // SAFETY: Base16Lower is the cow-protocol canonical multibase and
        // is unconditionally supported by Cid::to_string_of_base. This
        // expect documents the static infallibility rather than tolerating
        // caller input.
        cid.to_string_of_base(multibase::Base::Base16Lower)
            .expect("Base16Lower encoding is always supported by Cid::to_string_of_base")
    }

    /// Parses a canonical `CIDv1` multibase string back into an
    /// `AppDataHash`.
    ///
    /// The accepted shape is exactly the shape that
    /// [`AppDataHash::to_cid`] emits: `CIDv1`, raw codec (`0x55`),
    /// keccak-256 multihash (`0x1b`), 32-byte digest, multibase-encoded
    /// in lowercase base16. Every other shape is rejected at the decoder
    /// boundary. The cow protocol layer integration at
    /// `crates/app-data/src/cid.rs::cid_to_app_data_hex` accepts the same
    /// shape; the two parsers are byte-for-byte equivalent on every input.
    ///
    /// # Errors
    ///
    /// Returns [`CoreError::InvalidCid`] when the input is not a parseable
    /// CID string, is not `CIDv1`, uses a multicodec other than raw
    /// (`0x55`), uses a multihash other than keccak-256 (`0x1b`), or
    /// carries a digest whose length is not 32 bytes. The error variant
    /// is opaque by design — the upstream cow protocol surface does not
    /// emit any other shape, so consumers that need finer-grained
    /// diagnostics parse the input through the `cid` crate directly.
    #[must_use = "decoded AppDataHash carries the only signal of parse success"]
    pub fn try_from_cid(value: &str) -> Result<Self, CoreError> {
        const RAW_CODEC: u64 = 0x55;
        const KECCAK_256_CODE: u64 = 0x1b;

        let parsed = cid::Cid::try_from(value).map_err(|_| CoreError::InvalidCid)?;
        ...
        if parsed.hash().code() != KECCAK_256_CODE {
            return Err(CoreError::InvalidCid);
        }
        let digest = parsed.hash().digest();
        if digest.len() != Self::BYTE_LENGTH {
            return Err(CoreError::InvalidCid);
        }
