...
fn wrapped_and_protocol_addresses_match_pinned_upstream_values() {
    ...
    assert_eq!(
        mainnet_wrapped.address.as_str(),
    ...
    assert_eq!(
        gnosis_wrapped.address.as_str(),
        "0xe91D153E0b41518A2Ce8Dd3D7944Fa863463a97d"
    );
    assert_eq!(gnosis_wrapped.symbol, "WXDAI");

    assert_eq!(
        settlement_contract_address(SupportedChainId::Mainnet, CowEnv::Prod).as_str(),
        "0x9008D19f58AAbD9eD0D60971565AA8510560ab41"
    );
    assert_eq!(
        settlement_contract_address(SupportedChainId::Mainnet, CowEnv::Staging).as_str(),
        "0xf553d092b50bdcbddeD1A99aF2cA29FBE5E2CB13"
    );
    assert_eq!(
        vault_relayer_address(SupportedChainId::Mainnet, CowEnv::Prod).as_str(),
        "0xC92E8bdf79f0507f65a392b0ab4667716BFE0110"
    );
    assert_eq!(
        eth_flow_contract_address(SupportedChainId::Mainnet, CowEnv::Prod).as_str(),
