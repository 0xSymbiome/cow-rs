...
pub(crate) async fn execute_read_contract(
...
) -> Result<String, AlloyClientError> {
    ...
    serde_json::to_string(&json).map_err(|error| {
        AlloyClientError::Internal(format!(
            "JSON-encoding read_contract output failed: {error}"
        ))
    })
}

fn load_abi(abi_json: &str, method: &str) -> Result<JsonAbi, AlloyClientError> {
    serde_json::from_str::<JsonAbi>(abi_json).map_err(|error| {
        AlloyClientError::Validation(format!("failed to load ABI for `{method}`: {error}"))
    })
}
...
fn resolve_function<'abi>(
    abi: &'abi JsonAbi,
    method: &str,
) -> Result<&'abi Function, AlloyClientError> {
    let functions = abi.function(method).ok_or_else(|| {
        AlloyClientError::Validation(format!("ABI has no function named `{method}`"))
    })?;
    if functions.len() > 1 {
...
fn resolve_param_type(param: &Param, method: &str) -> Result<DynSolType, AlloyClientError> {
...
fn json_args_to_dyn_values(
    ...
    args: &Value,
    method: &str,
) -> Result<Vec<DynSolValue>, AlloyClientError> {
    match args {
        Value::Array(items) => {
        ...
        _ => Err(AlloyClientError::Validation(format!(
...
fn json_to_dyn_value(
    ...
    value: &Value,
    method: &str,
) -> Result<DynSolValue, AlloyClientError> {
    match ty {
        DynSolType::Address => {
        ...
        DynSolType::Bool => value.as_bool().map(DynSolValue::Bool).ok_or_else(|| {
            AlloyClientError::Validation(format!("method `{method}`: bool must be a boolean"))
        }),
        DynSolType::String => value
            .as_str()
            .map(|item| DynSolValue::String(item.to_owned()))
            .ok_or_else(|| {
                AlloyClientError::Validation(format!("method `{method}`: string must be a string"))
            }),
...
fn dyn_value_to_json(value: &DynSolValue) -> Result<Value, AlloyClientError> {
    match value {
        ...
        DynSolValue::CustomStruct { name, .. } => Err(AlloyClientError::Validation(format!(
            "unsupported ABI output shape: custom struct `{name}` not supported in this release"
        ))),
        other => Err(AlloyClientError::Validation(format!(
...
fn parse_u256(value: &Value, method: &str) -> Result<U256, AlloyClientError> {
    let raw = match value {
...
fn parse_i256(value: &Value, method: &str) -> Result<I256, AlloyClientError> {
    let raw = match value {
        Value::String(raw) => raw.clone(),
        Value::Number(number) => number.to_string(),
        _ => {
...
mod tests {
    ...
    fn dyn_value_to_json_rejects_custom_struct_with_validation_error() {
        ...
        assert!(matches!(error, AlloyClientError::Validation(_)));
    ...
    fn resolve_function_rejects_overloads() {
        ...
        assert!(matches!(
            ...
            Err(AlloyClientError::Validation(_))
        ));
    }
}
