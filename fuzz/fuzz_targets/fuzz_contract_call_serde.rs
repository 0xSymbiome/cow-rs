...
//! `async fn` wrapper `Provider::read_contract`. The fuzz crate does
//! not link an async executor for this target, so those helpers cannot
//! be driven directly here today. The gap and the committed future-target
//! names are tracked in `docs/audit/fuzz-coverage-audit.md`; when
//! async-runtime support is added for those targets, the dedicated
//! coverage joins the inventory under its own name without disturbing
//! this one.
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(call) = call.as_ref() {
        ...
        // `Provider::read_contract`. Round-trip parsing them as
