...
| --- | --- | --- |
| `seed-00-canonical-v-27.bin` | canonical | Canonical 65-byte hex signature with `v = 0x1b`, mirroring the first accept row of `parity/fixtures/ecdsa/v_normalization.json`. |
...
| `seed-07-adversarial-invalid-v.bin` | adversarial | 65-byte hex with `v = 0x02`, the documented rejected recovery byte from `parity/fixtures/ecdsa/v_normalization.json`. |
