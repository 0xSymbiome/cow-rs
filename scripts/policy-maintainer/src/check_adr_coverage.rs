...
pub fn run_with_writer(
    ...
    if errors.is_empty() {
        Diagnostic::info(
            "PM8000",
            format!("ADR coverage map covers {} accepted ADR(s)", statuses.len()),
        )
    ...
    for error in &errors {
        let diagnostic = match args.mode {
            Mode::Informational => Diagnostic::warn("PM8001", error),
            Mode::Blocking => Diagnostic::error("PM8001", error),
        };
