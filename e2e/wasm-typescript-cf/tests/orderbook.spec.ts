...
describe("Worker wasm helpers", () => {
  test("returns the wasm version", async () => {
    const response = await SELF.fetch("https://example.test/version");
  ...
  test("returns supported chains", async () => {
    const response = await SELF.fetch("https://example.test/chains");
  ...
  test("returns a CID from app-data hash", async () => {
    const response = await SELF.fetch("https://example.test/cid");
  ...
  test("returns an order UID", async () => {
    const response = await SELF.fetch("https://example.test/uid");
